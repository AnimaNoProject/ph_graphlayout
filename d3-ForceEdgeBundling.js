...
(function () {
    d3.ForceEdgeBundling = function () {
        ...
        function edge_as_vector(P) {
            return {
                'x': data_nodes[P.target.index].x - data_nodes[P.source.index].x,
        ...
        function edge_length(e) {
            ...
            if (Math.abs(data_nodes[e.source.index].x - data_nodes[e.target.index].x) < eps &&
                Math.abs(data_nodes[e.source.index].y - data_nodes[e.target.index].y) < eps) {
                return eps;
            }
        ...
        function position_compatibility(P, Q) {
            var lavg = (edge_length(P) + edge_length(Q)) / 2.0;
            var midP = {
                'x': (data_nodes[P.source.index].x + data_nodes[P.target.index].x) / 2.0,
                'y': (data_nodes[P.source.index].y + data_nodes[P.target.index].y) / 2.0
            };
            var midQ = {
                'x': (data_nodes[Q.source.index].x + data_nodes[Q.target.index].x) / 2.0,
        ...
        function edge_visibility(P, Q) {
            var I0 = project_point_on_line(data_nodes[Q.source.index], {
                'source': data_nodes[P.source.index],
                'target': data_nodes[P.target.index]
            });
            var I1 = project_point_on_line(data_nodes[Q.target.index], {
                'source': data_nodes[P.source.index],
                'target': data_nodes[P.target.index]
